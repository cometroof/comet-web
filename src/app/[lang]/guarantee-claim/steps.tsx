...
export default function Guarantee__Steps({
  ...
  return (
    <div className="flex flex-col lg:flex-row mt-10 lg:mt-20 pl-5 lg:pl-0">
      {loops.map((l, i) => (
        <div
          key={i}
          className="border-l-[3px] border-l-primary last:border-l-transparent lg:border-l-0  lg:border-t-[3px] lg:border-t-primary lg:last:border-t-transparent p-5 pl-7 pt-0 pb-10 lg:pb-5 lg:pl-5 lg:pt-16 relative group"
        >
          {/*{i < loops.length - 1 && (*/}
          <div className="size-7 lg:size-9 rounded-full bg-primary absolute left-0 -translate-x-[55%] top-0 lg:translate-x-[0] lg:-translate-y-[50%] text-app-white flex items-center justify-center"></div>
          ...
          <div className="mt-1 lg:mt-4 text-subheading h-auto lg:h-12">
            {l.name}
          </div>
          <div className="mt-1.5 lg:mt-2.5 text-caption">{l.description}</div>
        </div>
      ))}
    </div>
  );
